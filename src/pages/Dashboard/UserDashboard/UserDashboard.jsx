...
const UserDashboard = () => {
  ...
  const { data: participated = [] } = useQuery({
    ...
    queryFn: async () => {
      const res = await axiosSecure.get(
        `/contest/participated/${user.email}`
      );
      return res.data;
    },
  });
  ...
  /* ------------------------------------
      SAFE SORT (undefined deadline fix)
  -------------------------------------*/
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-10">
      ...
      <div className="text-center py-10 bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-3xl shadow-xl">
        ...
        <p className="mt-3 text-xl">
          Track your contest journey & achievements
        </p>
        <Link
          to="/dashboard/profile"
          className="btn btn-outline btn-white mt-6"
        >
          Edit Profile
        </Link>
      </div>
            ...
            <tbody>
              {sortedParticipated.map((c, i) => (
                <tr key={c._id} className="hover:bg-purple-50 transition">
                  ...
                  <td>${c.entryFee || 0}</td> {/* <-- fixed */}
                  <td className="text-green-600 font-bold">${c.prize || 0}</td>
                  <td>
                    {c.deadline
                      ? format(new Date(c.deadline), "dd MMM yyyy, h:mm a")
                      : "No deadline"}
                  </td>
                  <td>
        ...
        {won.length === 0 ? (
          <div className="text-center py-16">
            ...
            <p className="mt-4 text-gray-400">
              Your first victory is just around the corner
            </p>
          </div>
        ) : (
